...
function LandingContent() {
  ...
  const handleDeleteAccount = async () => {
    if (confirm('⚠️ ATENÇÃO: Tem certeza absoluta que deseja APAGAR sua conta? Esta ação é irreversível e excluirá todas as suas chaves e acessos do nosso banco de dados. O atalho Zimbroo no seu celular e o sistema DEIXARÃO de funcionar imediatamente.')) {
      const secondConfirm = confirm('Por segurança, confirme mais uma vez: deseja mesmo APAGAR e excluir seus dados permanentemente?');
      ...
      try {
        ...
        if (data.success) {
          alert('🗑️ Seus dados foram permanentemente removidos. O sistema do Hub Financeiro foi desativado para a sua conta.');
          ...
          // Reseta estados do checklist
          setNotionConnected(false);
          setShortcutSaved(false);
          setTemplateId(null);
          setActiveAccordion(null);
          
          // Desloga do Google Auth
          auth.signOut();
        } else {
          alert(`Erro ao excluir conta: ${data.message}`);
        }
      } catch (e) {
        alert('Erro de conexão ao tentar apagar sua conta. Tente novamente em breve.');
      }
    }
  };
  ...
  return (
    <main className="main" style={{ justifyContent: 'flex-start', paddingTop: '6rem' }}>
      ...
      {user && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.2rem', marginTop: '3rem' }}>
